...
app.post('/api/generate-daily-tasks', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const prompt = `Студент сам написал себе такой план поступления. Разбей ЕГО задачи из плана на конкретные ежедневные шаги — маленькие, выполнимые за один день действия (например, "заполнить один раздел анкеты", а не "подать документы"). Дай примерно 1-2 таких шага на каждую задачу плана, но не больше 15 шагов всего. Ответь ТОЛЬКО JSON.
...
Используй разные цвета образов для трёх специальностей.
${LANG_RULE}
Ответь ТОЛЬКО JSON.`;
...
${LANG_RULE} (названия университетов/городов/стран — на языке оригинала, всё остальное — по-русски)
Ответь ТОЛЬКО JSON.`;
...
${LANG_RULE} (даже если сам документ был на другом языке — переводи извлечённые задачи на русский)
Ответь ТОЛЬКО JSON.`;
